...
FleetStatesVisualizer::FleetStatesVisualizer(const rclcpp::NodeOptions &options)
    : Node("fleet_states_visualizer", options), _next_available_id(0) {
  RCLCPP_INFO(this->get_logger(), "Configuring fleet_states_visualizer...");

  _current_level = this->declare_parameter("initial_map_name", "L1");
  RCLCPP_INFO(this->get_logger(), "Setting parameter initial_map_name to %s",
              _current_level.c_str());
  ...
  _param_sub = this->create_subscription<RvizParam>(
      "rmf_visualization/parameters", rclcpp::SystemDefaultsQoS(),
  ...
  _fleet_sub = this->create_subscription<FleetState>(
      "/fleet_states", qos, [=](FleetState::ConstSharedPtr msg) {
        using RobotState = rmf_fleet_msgs::msg::RobotState;
        using Location = rmf_fleet_msgs::msg::Location;

        auto set_body_pose = [](const Location &loc, Marker &marker) {
        ...
        auto set_nose_pose = [](const Location &loc, const double radius,
                                Marker &marker) {
        ...
        auto set_text_pose = [](const Location &loc, const double radius,
                                Marker &marker) {
        ...
        auto fill_markers = [&](const RobotState &state, const double radius,
                                const std::size_t id,
          ...
          // ADDED BY THE CONSTRUCT END
          RCLCPP_DEBUG(this->get_logger(), "ROBOT ID==%s ", fleet_name.c_str());

          if (fleet_name == "box_bot") {
            body_marker.type = body_marker.CUBE;
            body_marker.type = body_marker.MESH_RESOURCE;
            body_marker.mesh_use_embedded_materials = true;
            body_marker.mesh_resource =
                "package://box_bot_description/meshes/box_bot_1.dae";
          ...
          } else if (fleet_name == "barista") {
            body_marker.type = body_marker.CUBE;
            body_marker.type = body_marker.MESH_RESOURCE;
            body_marker.mesh_use_embedded_materials = true;
            body_marker.mesh_resource = "package://barista_description/meshes/"
                                        "barista_single_mesh/barista.dae";
            // TODO(YV): Get the color to match that of the navigation graph
            body_marker.color.r = 0.0;
            body_marker.color.g = 0.0;
            body_marker.color.b = 0.0;
            ...
            body_marker.type = body_marker.MESH_RESOURCE;
            body_marker.mesh_use_embedded_materials = true;
            body_marker.mesh_resource = "package://dummybot_description/meshes/barista/dummy.dae";
            // TODO(YV): Get the color to match that of the navigation graph
            body_marker.color.r = 0.0;
            body_marker.color.g = 0.0;
            body_marker.color.b = 0.0;
            body_marker.color.a = 0.0;
          } else if (fleet_name == "cleaner") {
            body_marker.type = body_marker.CUBE;
            body_marker.type = body_marker.MESH_RESOURCE;
            body_marker.mesh_use_embedded_materials = true;
            body_marker.mesh_resource = "package://dummybot_description/meshes/barista/cleaner.dae";
            // TODO(YV): Get the color to match that of the navigation graph
            body_marker.color.r = 0.0;
            body_marker.color.g = 0.0;
            body_marker.color.b = 0.0;
        ...
        for (const auto &robot : msg->robots) {
          ...
          fill_markers(robot, radius, _ids[robot.name], msg->name,
                       *marker_array);
        }
  ...
  RCLCPP_INFO(this->get_logger(), "Running fleet_states_visualizer...");
}
